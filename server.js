...
routProducts.delete('/:id', async (req, res) => {
    const id = req.params.id
...
routCarrito.post('/:id/products', async (req, res) => {
    const id = req.params.id
    const product = req.body
    await carritos.postData(id, product)
    res.send('product agregado al carrito')
    // incorpora productos en un carrito segun su id
})

routCarrito.delete('/:id/productos/:id_prod', async (req, res) => {
